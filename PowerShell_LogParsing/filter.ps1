...
{
$dpath = "C:\"
(Get-ChildItem -Path $dpath -recurse -Directory | select Fullname).FullName.replace("\","\/") | out-file C:\ProgramData\soalog\path.txt
...
(Get-ChildItem -Path $fpath -recurse -file | select Fullname).FullName.replace("\","\/") | out-file C:\ProgramData\soalog\path.txt -Append
$dpath.Dispose()
$fpath.Dispose()
}
else
...
$dpath = "C:\"
(Get-ChildItem -Path $dpath -recurse -Directory | select Fullname).FullName.replace("\","\/") | Where-Object {$_ -notin $compare} | out-file C:\ProgramData\soalog\path.txt -Append
"C:\" | Where-Object {$_ -notin $compare} | out-file C:\ProgramData\soalog\path.txt -Append

$fpath = "C:\ProgramData\soalog", "C:\Windows\soa", "C:\Users\$env:username\AppData\Local\Kakao", "C:\Program Files (x86)\Kakao\KakaoTalk"
(Get-ChildItem -Path $fpath -recurse -file | select Fullname).FullName.replace("\","\/") | Where-Object {$_ -notin $compare} | out-file C:\ProgramData\soalog\path.txt -Append
$dpath.Dispose()
$fpath.Dispose()
}
